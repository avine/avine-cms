 ...
 * Version en langue FRANCAISE (variante : Belgique)
...
function conversion($n,$cpt){
	...
	$nombre=array('', 'dix', 'vingt', 'trente', 'quarante', 'cinquante', 'soixante', 'septante', 'quatre-vingt', 'nonante');
	//Conversion
	if($D==0){$valeur=$chiffre[$U];} 							// 1 à 10 (ne retourne pas "zéro" pour 0)
	elseif($D==1){$valeur=$chiffre[($U+10)];} 					// 11 à 19
	elseif(($D>=2 && $D<=7) || $D==9){							// 20 à 79 + 90 à 99
	...
	elseif($D==8){												// 80 à 89
		if($U==0){
			...
			else{$valeur=$nombre[$D];}							// 80 000 => quatre-vingt mille (cas particulier: pas de "S")
		}						
	...
	elseif($C!=0){												// 2xx à 9xx
		if(strlen($valeur)==0){										// 200, 300, 400, 500, 600, 700, 800, 900
			if($cpt==1){$valeur=$chiffre[$C].' cent';}					// cas particulier (ex: 300 000 : trois cent mille)
			else{$valeur=$chiffre[$C].' cents';}						// 300 ... le reste (ex: 300 000 000 : trois cents millions)
		}
